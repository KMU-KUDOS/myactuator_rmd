# MyActuator RMD Motor Driver (ROS2 Humble)

C++ library and ROS2 Humble integrated driver for RMD series servo motors from GYEMS/MyActuator using the V1.61 CAN bus protocol.

## Overview

This project aims to provide a reliable and high-performance solution for controlling RMD V1.61 servo motors in real-time Linux environments, with a focus on ROS2 Humble integration via `ros2_control`.

## Features (Planned)

- Full support for RMD V1.61 CAN protocol (single and multi-motor commands).
- Real-time capable C++ core library.
- ROS2 Humble LifecycleNode integration.
...
(License details to be added - e.g., Apache 2.0 or MIT)
