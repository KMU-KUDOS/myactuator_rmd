...
int main() {
  ...
  try {
    ...
    std::cout << "[Initial Encoder (0x90)] Pos: " << initial_encoder.position << ", Raw: " << initial_encoder.raw_position << ", Offset: " << initial_encoder.offset << '\n';
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(200)); // Wait after writing
    ...
    if (motor.writeEncoderOffset(new_offset, written_offset)) {
      std::cout << " -> Successfully sent Write Encoder Offset command. Motor reported written offset: "<< written_offset << '\n';
    } else {
    ...
    if (motor.clearErrorFlag(status_after_clear)) {
      std::cout << " -> Successfully sent Clear Error Flag command. Current Status:" << '\n';
    ...
    // Save current location as zero (0x19) - Caution! Write ROM, reboot required
    // Be very careful when connecting the actual motor. The zero point will change.
    std::cout << '\n' << "Testing Write Position As Zero to ROM (0x19)..." << '\n';
    uint16_t zero_offset_written = 0;

    if (motor.writePositionAsZero(zero_offset_written)) {
      std::cout << " -> Successfully sent Write Position As Zero command. Reported new offset: " << zero_offset_written << '\n';
      std::cout << " -> !!! Motor requires restart for this change to effect !!!" << '\n';
    } else {
    ...
    // Make sure the motor is securely fastened and that there are no obstacles around it
    // Power off the motor before testing, and have an emergency stop available during testing
    // !!! Caution !!!
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(500)); // Wait long enough for it to turn off
    ...
    // Position Control Test 2 (Multiple Turns, Speed Limit) - Example: 0 degree movement
    ...
    std::this_thread::sleep_for(std::chrono::seconds(3)); // Time to reach the target location (long enough)

    // Position Control Test 4 (Single Turn, Directional, Speed Limit) - Example: 180 degree travel (CCW)
    std::cout << '\n' << "Testing Position Control 4 (0xA6)... Target: 0 deg (CCW). Max Speed 0 dps" << '\n';
    uint16_t angle_sp_single_deg100 = 0 * 100;
    uint16_t max_speed_single_dps = 0;

    if (motor.setPositionControl4(angle_sp_single_deg100, v161_motor_control::types::SpinDirection::COUNTER_CLOCKWISE, max_speed_single_dps, feedback)) {
      std::cout << " -> Position 4 command sent. Feedback:" << '\n';
      printStatus2(feedback);
    } else {
      std::cerr << " -> Failed to send Position Control 4 command" << '\n';
    }
    std::cout << " -> Moving to 0 deg (CCW) for 4 seconds (adjust time as needed)..." << '\n';
    std::this_thread::sleep_for(std::chrono::seconds(4));

    // Torque Control Test - Caution: Can be dangerous depending on motor load and model!
    // Example: Small torque value (equivalent to 0.1 A) - value needs to be calculated
    int16_t torque_sp = static_cast<int16_t>(0.1f * (2000.0f / 32.0f)); // 0.1A -> approx 62
    std::cout << '\n' << "Testing Torque Control (0xA1)... Target: " << torque_sp << " (~0.1A)" << '\n';
