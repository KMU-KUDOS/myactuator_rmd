...
/**\class Frame
 * \brief
 *    Class for a frame of the CAN bus protocol, see
 * https://en.wikipedia.org/wiki/CAN_bus This class replaces the Linux SocketCAN
 * frame struct in order to be more portable
 */
class Frame {
 public:
  /**\fn Frame
   ...
   */
  constexpr Frame(std::uint32_t const can_id,
                  std::array<std::uint8_t, 8> const& data) noexcept;
  Frame() = delete;
   ...
   *    The data to be transmitted to the CAN node
   */
  [[nodiscard]] constexpr std::array<std::uint8_t, 8> const& getData()
      const noexcept;
...
constexpr Frame::Frame(std::uint32_t const can_id,
                       std::array<std::uint8_t, 8> const& data) noexcept
