...
/**\class Node
 *  \brief
 *     Base class for sending and receiving CAN frames over SocketCAN with a
...
class Node {
 public:
   ...
   * communication and sets up debugging
   *
   * \param[in] ifname
   ...
   *
   * \param[in] can_ids
   *    The CAN id that should be accepted (discarded if \p is_invert set to
   * true)
   * \param[in] is_invert
  ...
  /**\fn setErrorFilters
   * \brief
   *    Set error filters for the socket. We will only receive error frames if
   * we explicitly activate it!
   *
   * \param[in] is_signal_errors
